...
async function fetchfeatureddata(displayfeatueddata) {
    ...
    // console.log(rnum)
    try {
        let featuredres = await fetch(`https://indiatoday-server.onrender.com/articles?_start=${rnum}&_limit=3`)
        let featureddata = await featuredres.json()
        let topres = await fetch(`https://indiatoday-server.onrender.com/articles?_start=${rnum}&_limit=40&country=India`)
        let topdata = await topres.json()

        let feedsres = await fetch(`https://indiatoday-server.onrender.com/articles?_start=${rnum}&_limit=100`)
