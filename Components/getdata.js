...
async function fetchfeatureddata(displayfeatueddata) {
    ...
    // console.log(rnum)
    try {
        let featuredres = await fetch(`https://india-today-news-api.herokuapp.com/articles?_start=${rnum}&_limit=3`)
        let featureddata = await featuredres.json()
        let topres = await fetch(`https://india-today-news-api.herokuapp.com/articles?_start=${rnum}&_limit=40&country=India`)
